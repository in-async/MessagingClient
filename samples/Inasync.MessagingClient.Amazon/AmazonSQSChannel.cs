...
namespace Inasync.MessagingClient.Amazon {
    ...
    public sealed class AmazonSQSChannel : IMessageChannel<string> {
        ...
        /// <param name="pollingIdleTime"><see cref="SubscribeAsync(MessageChunkConsumerFunc{string}, CancellationToken)"/> を実行中、受信メッセージが無い場合に待機する時間。</param>
        /// <param name="logger">処理の途中経過を通知するロガー。主に <see cref="SubscribeAsync(MessageChunkConsumerFunc{string}, CancellationToken)"/> 用。</param>
        ...
        /// <summary>
        /// <see cref="IOutputMessageChannel{TMessage}.SubscribeAsync(MessageChunkConsumerFunc{TMessage}, CancellationToken)"/> の実装。
        /// <para>
        /// SQS キューからメッセージを受信し、消費したメッセージをキューから削除します。
        /// </para>
        /// </summary>
        public Task SubscribeAsync(MessageChunkConsumerFunc<string> consumer, CancellationToken cancellationToken) {
            ...
            return Task.Run(async () => {
                while (true) {
                    ...
                    var consumeResults = new bool[messages.Count];
                    try {
                        await consumer(messages.Select(x => x.Body).ToArray(), consumeResults, cancellationToken).ConfigureAwait(false);
                    }
                    catch (OperationCanceledException) { throw; }
